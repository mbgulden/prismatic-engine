---
type: Architecture
title: Prismatic Engine — Architecture Reference
description: Module map, public API surface, two-dispatcher model, and review-loop/rate-limit references for the prismatic-engine.
resource: okf/architecture.md
tags: [architecture, prismatic-engine, modules, two-dispatcher]
timestamp: 2026-06-19T10:30:00Z
linear_issue: GRO-2032
git_repo: mbgulden/prismatic-engine
git_path: okf/architecture.md
...
**Status:** Tier 4 deliverable (also mirrored as OKF for Tier 5a).
**Audience:** Anyone touching `prismatic/`, integrating a new agent, or
debugging cross-module behavior.
**Canonical OKF version:** this file. The pre-OKF version lives at
`docs/architecture.md` and will be deprecated when Tier 5b rolls out.

This doc links out to source files rather than restating them. The engine
is the durable source of truth; this is the map of it.

## What is the Prismatic Engine?

A harness-agnostic Python library for orchestrating multi-agent work against
Linear. Replaces ad-hoc cron + shell glue with:

- A **state machine** for orchestration modes (`mode_switch.py`).
- A **dispatcher** that pulls issues from Linear, decides which agent handles
  them, and applies lane transitions (`dispatcher.py`).
- A **provider abstraction** so adding GitHub/Jules/Telegram/etc. is one new
  file under `prismatic/providers/` and one capability entry.
- **Rate-limit and budget enforcement** so a runaway cron cannot exhaust
  Linear API quota. See [`./linear-rate-limit.md`](./linear-rate-limit.md).

The engine is not a replacement for Hermes; it is a library that **Hermes
profiles call into**. The profile handles auth, model choice, and chat-side
coordination; the engine handles Linear state, agent lane transitions, and
...
| `doctor.py` | Health-check + capability report | Always-on sanity check |
...
Pre-registered: `linear`, `vcs.github`, `agy`, `jules`, `telegram`, `schedule`,
`artifact`. See `prismatic/capabilities/registry.py`.

## Two-Dispatcher Model

The orchestrator profile and the engine repo **both run a dispatcher today**.
They are not duplicates; they cover different scopes.

### Profile dispatcher (`~/.hermes/profiles/orchestrator/scripts/agent_dispatcher.py`)

- **Authoritative for the review loop.** Holds the GRO-2024 bypass-detection
  that ensures no agent reaches `agent:done` without Fred verification.
...
**Single source of truth for the loop:** the orchestrator profile, until the
engine dispatcher ports bypass detection.

See [`growthwebdev-knowledge/okf/decisions/okf-adoption.md`](https://github.com/mbgulden/growthwebdev-knowledge/blob/main/okf/decisions/okf-adoption.md)
for the broader OKF decision. See [`./review-loop-canonical.md`](./review-loop-canonical.md)
for the review-loop standard.
...
Only `agent:fred` may move to `agent:done`. The dispatcher enforces this at every
transition point. See [`./review-loop-canonical.md`](./review-loop-canonical.md).
...
**Lint script:** `scripts/check_linear_cron_rate.sh` — fails CI if total
expected cron usage > 2000 req/hour.

**Doctor integration:** `prismatic doctor` always prints `[Linear] Rate limit`
section.

See [`./linear-rate-limit.md`](./linear-rate-limit.md) for the full standard.

## AGY Integration

AGY CLI cannot write artifacts to `/tmp/` (sandbox restriction). The wrapper
at `bin/launch_agy_with_artifact.py` handles this:
...
Synced in `agent_dispatcher.py::LABEL_TO_MODEL`, `agent_dispatcher.py::AGENT_CONFIG`,
`agy_sandbox_event_supervisor.py::DEFAULT_MODEL`, and native AGY config.
...
| 3 | In-flight inventory refreshed | GRO-2031 |
...
2. Add a row to `prismatic/capabilities/registry.py::pre_register` if it needs
   an env-var check.
3. Add to `AGENT_CONFIG` in `agent_dispatcher.py` (profile) with
   `mode: signal` or `mode: print`, `next_label: agent:fred` (for peer-review
   chains) or `next_label: agent:agy` (for worker chains).
...
6. Update `agent-label-catalog.md` and
   `skills/orchestration/orchestrator-delegation-discipline/references/quality-loop-enforcement.md`.

## Adding a New Capability

1. Create `prismatic/capabilities/<name>.py` exposing a `<Name>Capability` class.
2. Implement `has_credentials(self) -> bool`, `verify(self) -> bool`,
   `emit_event(self, event_type, payload) -> None`.
3. Register in `prismatic/capabilities/__init__.py` and `pre_register()`.
4. Verify with `prismatic doctor --provider <name>`.

## Adding a New Provider

1. Create `prismatic/providers/<name>.py` with a class implementing the
   provider protocol.
...
| Doc | Path |
|---|---|
| Review loop canonical codification | [`./review-loop-canonical.md`](./review-loop-canonical.md) and the canonical hub version |
| Linear rate-limit codification | [`./linear-rate-limit.md`](./linear-rate-limit.md) and the canonical hub version |
| Profile ↔ engine dispatcher | `docs/profile-vs-engine-dispatch.md` (this repo, pre-OKF) |
...
- `get_agy_schedules()` and `get_jules_schedules()` are mocked until AGY/Jules
  expose real `/schedule` endpoints.
...
2. Re-run `find prismatic tests -maxdepth 3 -type f -name '*.py' | grep -v __pycache__`
   and update the module map.
3. Run `prismatic doctor` before pushing.
4. Open PR; AGY peer review before merge.

Last updated 2026-06-19 by Fred, post-Tier 1/2/3/4 work (GRO-2032). OKF
conversion 2026-06-19 (GRO-2039).
