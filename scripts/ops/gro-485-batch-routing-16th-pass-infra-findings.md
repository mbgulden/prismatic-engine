# GRO-484..502 batch routing — 16th pass infra findings (cron 2026-06-29 ~20:46Z)

## TL;DR

Pass number: **16** (sixteenth ops audit doc on the GRO-484..502 misroute
batch; follows the 1st–15th pass docs at
`scripts/ops/gro-485-batch-routing-{1..15}-pass-infra-findings.md`).
...
comment at `2026-06-29T18:33:44.482Z` (~2.22h old) names all 10 batch
...
Delta vs prior pass (20:30Z, 15th): **STABLE path-2 SUPPRESS per r59 +
r150**. Scanner feed **byte-identical** to the 15 prior passes
...
Michael dequeue marker pinned, unchanged). Gap since 15th-pass audit
doc commit (20:30:49Z) is **~16 min**, well under the 30-min
probe-skip floor.

All 5 byte-identical probe conditions hold vs the 15th pass (20:30Z):
...
3. ✅ GRO-485 first-Michael-comment `2026-06-29T09:25:47.467Z` (dequeue
   ...
   `2026-06-29T18:33:44.482Z` (r130, ~2.22h old) names all 10 batch
   IDs, has standing cure, has lane map. **No fresh fan-noise
   finalize-evidence discharge since `2026-06-29T15:18:38.896Z`** —
   gap now **~5h 28m**, asymptoting per Pass-11/12/14/15 observation
   toward ~5h wrapper-side ceiling.
...
5. ✅ `swarm.js status` → no active locks (unchanged from 15th pass).

## Lane disposition (unchanged from 15 prior passes)
...
probes entirely and document the skip explicitly. The 15th-pass audit
doc (20:30:49Z commit time, ~16m old) is fresh; GPU/disk/locks/Tailscale
all unchanged from 15th-pass baselines; fan-noise discharge gap
asymptoting. **Probes NOT re-run this pass** — last-known-good
baselines (per 15th-pass doc):

- GPU node (100.78.237.7): offline 8d 22h+ monotonic (verified
  Pass-12)
- Disk: unchanged from 15th pass
...
| **16** | **2026-06-29 20:46Z** | **18:33:44Z** | **~5h 28m** | **SILENT** | **Yes (<30m)** | **This pass** |

## Six consecutive SILENT passes confirms steady-state

Passes 11, 12, 13, 14, 15, 16 all returned `verdict: SILENT` from the
...
The git log on `ned/gro-485-triage-pass-1` (16 commits today) is the
...
Anchor from `18:33:44Z` (r130 post-arm) is **2.22h** old, well under
...
- Did NOT push the branch (Michael decides; this is a sustained-SUPPRESS
  batch).
- Did NOT run infra probes (Pass-12 protocol satisfied; 15th-pass
...
- Verified byte-identity vs the 15th-pass audit doc (5 conditions
  all hold).
- Ran the scorer as 1st action — returned `verdict: SILENT` (anchor
  18:33:44Z, 2.22h old, qualifies on `names_all_batch_ids ×
  has_standing_cure × has_lane_map × age<6h`).
- Skipped probes per Pass-12 protocol (prior audit doc fresh at
  ~16m, well under 30m floor).
- Authored this 16th-pass audit doc at the canonical location
  (`scripts/ops/gro-485-batch-routing-16th-pass-infra-findings.md`).
- Will commit on the continued `ned/gro-485-triage-pass-1` branch
...
— Ned (autonomous cron, recurring-batch 16th-pass acknowledgment, not a blocker)