# GRO-484..502 batch routing — 14th pass infra findings (cron 2026-06-29 ~20:23Z)

## TL;DR

Pass number: **14** (fourteenth ops audit doc on the GRO-484..502 misroute
batch; follows the 1st–13th pass docs at
`scripts/ops/gro-485-batch-routing-{1..13}-pass-infra-findings.md`).
...
comment at `2026-06-29T18:33:44.482Z` (1.81h old) names all 10 batch IDs,
...
Delta vs prior pass (20:11Z, 13th): **STABLE path-2 SUPPRESS per r59 +
r150**. Scanner feed **byte-identical** to the 13 prior passes
...
All 5 byte-identical probe conditions hold vs the 13th pass (20:11Z):
...
3. ✅ GRO-485 first-Michael-comment `2026-06-29T09:25:47.467Z` (dequeue
   ...
   `2026-06-29T18:33:44.482Z` (r130, 1.81h old) names all 10 batch IDs,
   has standing cure, has lane map. **No fresh fan-noise
   finalize-evidence discharge since `2026-06-29T15:18:38.896Z`** — gap
   now **~5h 05m**, asymptoting per Pass-11/12 observation toward
...
5. ✅ `swarm.js status` → no active locks (unchanged from 13th pass).

## Lane disposition (unchanged from 13 prior passes)
...
probes entirely and document the skip explicitly. The 13th-pass audit
doc (20:12:30Z commit time, ~11m old) is fresh; GPU/disk/locks/Tailscale
all unchanged from 13th-pass baselines; fan-noise discharge gap
asymptoting. **Probes NOT re-run this pass** — last-known-good
baselines (per 13th-pass doc):

- GPU node (100.78.237.7): offline 8d 21h+ monotonic (verified
  Pass-12)
- Disk: unchanged from 13th pass
...
(mtime 2026-06-29T18:23Z, ~1h before this pass). This matches the
r133 wrapper-side pattern but the file is **untracked**, not on
the branch, so no `git revert` is required. Per the r133 recipe:
DO NOT commit this file on `ned/gro-485-triage-pass-1` — it's out of
lane (`okf/operations/` is owned by agy/kai per PRISMATIC_ENGINE.yaml,
not ned). Left alone; the pre-push hook would block any attempt.
...
Anchor from `18:33:44Z` (r130 post-arm) is **1.81h** old, well under
...
Passes 9, 10, 11, 12, 13, 14 all returned `verdict: SILENT` from the
...
The git log on `ned/gro-485-triage-pass-1` (14 commits today) is the
...
- No touch of the untracked `okf/operations/2026-06-30-...` file
  (out-of-lane; r133 mitigation).

## What I did do

- Re-queried all 10 issues via scorer for state/labels/comments —
  confirmed identical to 13th pass.
- Ran `anchor_5a5_item3_scorer.py` as 1st action per skill —
  returned `verdict: SILENT` (anchor 18:33:44Z, 1.81h old, qualifies
  on `names_all_batch_ids × has_standing_cure × has_lane_map ×
  age<6h`).
- Wrote this 14th-pass audit doc — durable evidence that Ned saw the
...
Same as 13 prior passes. Either relabel the 10 issues OR fix the
dispatcher scanner.