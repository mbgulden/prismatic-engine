...
Hermes' cron injection scanner (`_CRON_THREAT_PATTERNS`) scans the assembled prompt (user prompt + loaded skill content) for threat patterns. **Loading 42 skills triggered `read_secrets`; loading 8 skills triggered `destructive_root_rm`.** Any skill containing sample commands, credential-management instructions, or API key handling patterns can trigger false positives.
...
6. **Delete nudge files** — delete both `/tmp/trigger-fred-work` (if exists) and `/tmp/prismatic/nudge-*` (if any match this issue ID) to prevent the signal from retrying. Check both locations: `rm -f /tmp/trigger-fred-work /tmp/prismatic/nudge-*`
