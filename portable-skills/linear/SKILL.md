...
### @file alternative for complex queries

When the query has nested quotes, brackets, or needs shell escaping that's hard to get right, write the payload to a temp file and point curl at it:

```bash
# Write payload to file (avoids shell escaping headaches)
cat > /tmp/linear_query.json << 'EOF'
{"query": "query { issues(filter:{labels:{name:{eq:\"agent:fred\"}}, state:{name:{eq:\"Todo\"}}}, first:5) { nodes { id identifier title } } }"}
EOF
...
This is especially useful inside `execute_code` sandboxes where Python string escaping compounds with shell escaping. Use `write_file` to create the payload, then `terminal` with `-d @file`.
...
For multiline Markdown comments (task summaries with code blocks, headers, quotes), the inline approach fails. Use the `json.dumps()` + `@file` pattern documented in `references/multiline-comment-posting.md`.
...
See `references/filter-cookbook.md` for practical filter patterns (label, state-name, title-substring, and the `identifier`-as-filter gotcha).
...
- **Cron/API environment check:** In scheduled jobs, verify `LINEAR_API_KEY` from the same execution path you will use for the API call. If a Python sandbox or helper lacks the env var, retry via `terminal`/`curl` before treating Linear auth as unavailable; profile-injected credentials may be present in the shell even when a sandboxed helper does not inherit them.
- **Sort parameter is finicky.** The `sort` argument on `issues` takes `[IssueSortInput!]` — not a bare string and not `{field, direction}` objects. The schema is non-obvious and brittle. When chronological ordering matters, prefer fetching a larger batch (e.g., `first:20`) and sorting client-side rather than wrestling with the GraphQL sort. Dropping `sort` entirely and relying on `first:N` is often simpler and always works.
- **GraphQL field aliasing required for multiple single-issue lookups.** You can't have two un-aliased `issue(id:)` fields with different arguments in the same query — GraphQL returns `GRAPHQL_VALIDATION_FAILED: "Fields 'issue' conflict because they have differing arguments"`. Use aliases: `{ gro847: issue(id:"GRO-847") { ... } gro848: issue(id:"GRO-848") { ... } }`. Or make separate queries — simpler and avoids the alias syntax overhead in shell escaping.
- **Cron-job "KNOWN" lists are stale snapshots.** Cron job prompts may embed a known-state list that was accurate when the job was created but goes stale as issues move. Always query Linear directly — never trust embedded state annotations. This session: GRO-847, GRO-848, GRO-850 were all Done per Linear, despite the cron prompt claiming they were Todo.
